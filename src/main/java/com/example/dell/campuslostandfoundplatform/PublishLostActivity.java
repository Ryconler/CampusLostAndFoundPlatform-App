...
public class PublishLostActivity extends AppCompatActivity {
        ...
        publishLost.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(title.length()<3||title.length()>20){
                    Toast.makeText(PublishLostActivity.this,"标题应在3到20个字之间",Toast.LENGTH_SHORT).show();
                }
                else if(address.length()<2||address.length()>20){
                    Toast.makeText(PublishLostActivity.this,"地拾物地点应在2到20个字之间",Toast.LENGTH_SHORT).show();
                }
                else if(name.equals("")||name.length()>10){
                    Toast.makeText(PublishLostActivity.this,"联系人姓名不能为空且字数不超过10个字",Toast.LENGTH_SHORT).show();
                }
                else if(!detail.equals("")&&(detail.length()>100)){
                    Toast.makeText(PublishLostActivity.this,"详细描述不能超过100个字",Toast.LENGTH_SHORT).show();
                }
                else if(!mTel.matches()||tel.length()!=11){
                    Toast.makeText(PublishLostActivity.this,"手机号应为11位的数字",Toast.LENGTH_SHORT).show();
                }
                else if (!qq.equals("")&&(!mQq.matches()||qq.length()<5||qq.length()>10)){
                    Toast.makeText(PublishLostActivity.this,"qq号应为5-10位的数字",Toast.LENGTH_SHORT).show();
                }
                else {
                        ...
                        public void run() {
                            ...
                            try{
                                ...
                                if(result){
                                    finish();
                                    Looper.prepare();
                                    Toast.makeText(PublishLostActivity.this,"发布成功",Toast.LENGTH_SHORT).show();
                                    Looper.loop();
                                }
                                else {
                                    Looper.prepare();
                                    Toast.makeText(PublishLostActivity.this,"内部错误",Toast.LENGTH_SHORT).show();
                                    Looper.loop();
                                }
                            }
